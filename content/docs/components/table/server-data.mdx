...
<Tabs items={['Bloggers Table', 'Data Table']}>
...
import { Button } from "@/components/ui/button"
import { useBloggersUrlState } from "../hooks/use-bloggers-url-state"
import type { Blogger, BloggersTableParams } from "../models/blogger.model"
...
export const BloggersTable = () => {
  ...
  const [query] = useBloggersUrlState()
...
toast.success("Blogger deleted successfully.")
      ...
      // IF it was the very last blogger on the page, go back one page unless we are on the first page
      if (
        data &&
        data.length === successData.input.length &&
      ...
      const deletedIds = successData.input

      // Clear selected bloggers if they were deleted
      setSelectedBloggers(prev => prev.filter(b => !deletedIds.includes(b._id)))
    },
    onError: error => {
